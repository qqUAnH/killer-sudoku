...
import scalafx.scene.shape.{HLineTo, MoveTo, Path, VLineTo}
import scalafx.scene.text.Text



def CreateRectangle( color:Color) :shape.Rectangle =
  new shape.Rectangle:
    fill=color
    x = 0
    y = 0
...
def createStackPane(x:Int,y:Int, gridPane: GridPane) = new StackPane:
  ...
  var i = 0
    hover.onChange (
      (_,_,_) =>
      if i == 0 then
        rect.fill.update(square.color.darker())
      else
        rect.fill.update(square.color))
